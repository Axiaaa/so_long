...
int is_valid(t_so_long *so_long, int x, int y)
{
	...
	if (so_long->game.map[y / 44][x / 44] == '1')
		return (0);
	if (so_long->game.map[y / 44][x / 44] == 'C')
	{
		so_long->game.map[y / 44][x / 44] = '0';
		so_long->player.items++;
	}
	if (so_long->game.map[y / 44][x / 44] == 'E' && so_long->player.items == so_long->r_map.items)
	{
...
{
	if (keycode == XK_w)
		if (is_valid(so_long, so_long->player.cords.x, so_long->player.cords.y - 44))
			so_long->player.cords.y -= 44;
	if (keycode == XK_s)
		if (is_valid(so_long, so_long->player.cords.x, so_long->player.cords.y + 44))
			so_long->player.cords.y += 44;
	if (keycode == XK_a)
		if (is_valid(so_long, so_long->player.cords.x - 44, so_long->player.cords.y))
			so_long->player.cords.x -= 44;
	if (keycode == XK_d)
		if (is_valid(so_long, so_long->player.cords.x + 44, so_long->player.cords.y))
			so_long->player.cords.x += 44;
